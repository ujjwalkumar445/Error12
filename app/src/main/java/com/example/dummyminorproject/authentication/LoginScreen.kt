        ...
        when(v!!.id){
            ...
            R.id.login_button -> {
                if(TextUtils.isEmpty(emailEditTextView.text)){
                    ...
                    mAuth.signInWithEmailAndPassword(emailEditTextView.text.toString(),passwordEditTextView.text.toString())
                        .addOnCompleteListener {task ->
                            if (task.isSuccessful){
                                Log.d(ContentValues.TAG,"SignInWithEmail : Success")
                                updateUi()
                            } else {
                                Log.w(ContentValues.TAG,"UserWithEmail:failure",task.exception)
